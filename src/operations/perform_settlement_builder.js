...
export class PerformSettlementBuilder {
    ...
    static performSettlement(opts) {
        ...
        for (let i = 0; i < opts.settlementAssets.length; i++) {
            let settlementAsset = opts.settlementAssets[i];

            if (!BaseOperation.isValidAsset(settlementAsset.code)) {
                throw new Error("opts.settlementAssets["+i+"].code is invalid: " +
                    settlementAsset.code);
            }

            if (!BaseOperation.isValidAmount(settlementAsset.price)) {
                throw new Error("opts.settlementAssets["+i+"].price is invalid: " +
                    settlementAsset.price);
            }
        ...
        let opAttributes = {};
        opAttributes.body = xdr.OperationBody.performSettlement(
            new xdr.PerformSettlementOp(attrs));
        BaseOperation.setSourceAccount(opAttributes, opts);
        return new xdr.Operation(opAttributes);
    }

    static performSettlementOpToObject(result, attrs) {
        ...
        for (let i = 0; i < attrs.settlementAssets().length; i++) {
